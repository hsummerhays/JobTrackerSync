...
| `utils.py` | Shared status-ranking, merge, canonical lists, text/company normalization, classification, hashing, backups, and file-URI helpers used by the above |
...
- **Single shared-helpers module**: `utils.py` holds generic helpers (`normalize_string`, `path_to_file_uri`, `clean_company_name`, `normalize_ocr_spacing`, `word_boundary_pattern`, `hash_file`, `backup_file_if_exists`, `write_csv_atomic`), domain classification (`classify_job_type`, `classify_workplace`, `detect_provider`, `compute_priority`), integration helpers (`generate_calendar_url`, `create_vcard_entry`), and dedup/status-specific ones (`canonical_key`, `canonical_job_key`, `merge_delimited_field`, status ranking, canonical status collections) in one file rather than splitting into multiple micro-modules.
- **Roman Numeral Title Normalization**: `utils.normalize_title()` normalizes standalone Roman numeral level indicators (`I`, `II`, `III`, `IV`, `V`) to Arabic digits (`1`, `2`, `3`, `4`, `5`) in deduplication keys (`canonical_key()`, `canonical_job_key()`). This allows variant titles like `Senior Developer I` and `Senior Developer 1` to resolve to the same canonical key without clobbering the original displayed title.
...
- **Recovery Baseline Contract**: A point-in-time recovery baseline snapshot ([docs/stabilization_baseline.md](file:///c:/HughApps/JobTrackerSync/docs/stabilization_baseline.md)) is maintained locally alongside git tag `v1.3.2-recovery-baseline-20260812` to ensure production tracker end-to-end data contracts (such as exact row counts, deduplication, and score preservation) remain intact during stabilization refactors.

---

## Future Enhancements

- **CSV as a pure generated view, not an operational participant**: `jobs.db` is logically authoritative (see CHANGELOG v1.3.3), but `master_tracker.csv` has not yet become a purely disposable projection -- it still participates operationally in enough read/write paths (two separate writes, pre-write backups, `clean_existing_tracker`'s CSV-parsing/migration pass, deferred `processed_files` commits) that a database failure after a partial CSV/DB write can still leave the two diverged or PDFs marked processed prematurely. Making SQLite the sole owner of identity, workflow, audit history, and processed-file state, and regenerating the CSV atomically as a `SELECT * FROM jobs` projection whenever needed, would eliminate the dual-write/backup-both/reconcile dance and the migration logic entirely. This is a substantial refactor (touches every read/write path that currently targets the CSV) and should be scoped as its own migration plan rather than done incrementally.
