...
- **Company Suffix Matching & Location Compatibility**: Wrapped company suffix normalization (`normalize_company_for_matching()`) into ingestion duplicate matching and `clean_existing_tracker()`, resolving duplicate identity collisions (such as `Wheeler Machinery Co.` vs `Wheeler Machinery Company`). `locations_compatible()` in `dedup_utils.py` remains conservative: two clean, distinct locations (e.g. `Midvale, UT` vs `West Valley City, UT`) are never considered compatible, since that usually indicates two different postings. Malformed/unclean location values are still treated permissively (compatible with anything) to avoid false-negative rejections from bad OCR extraction.
- **Pre-Insertion Rejection Filters**: Enhanced `is_valid_company()` in `parse_jobs.py` with a `JOB_TITLE_ROLE_WORDS` validator that rejects pure job titles (e.g., `Senior Software Engineer`) captured in the company slot during card inversions. Added filter rules to reject benefit/culture text (`Vacation & Paid Time Off`, `Full-Time • Positive Culture & Values`), employment headers (`(Only on W2)`), UI navigation elements (`More jobs ➞ More remote jobs`), and generic AI acronyms (`GenAI`, `Generative AI`).
- **Whitelisted Valid Company Names**: Added explicit whitelisting for real employer names `fullstack` (e.g. `FullStack`) and `talentarchitect.com` to prevent false-positive rejections.
- **Same-Day Sighting Relisting Noise Reduction**: Guarded relisting note generation (`Re-listed on YYYY-MM-DD; originally seen YYYY-MM-DD.`) with a strict `true_last_date > true_first_date` check. Multi-source sightings on the same day now update source references silently without adding duplicate relisting notes. Updated relisting note stripping regex (`Re-listed.*?originally seen \d{4}-\d{2}-\d{2}`) to non-greedy matching so legacy same-day relisting notes are cleanly removed.
- **OCR Kerning Execution Sequence**: Adjusted the ordering of OCR kerning fixes (`West Valley`, `Technologies`) to run after general single-letter reassembly heuristics, ensuring four-fragment extractions (e.g. `W est V alley`) are fully reassembled.
...
- **Manual Score Override Preservation**: Manual scores set via CLI (`--add --fit-score`, `--update --fit-score`) or database overrides are preserved across `clean_existing_tracker()` sync passes and default `python parse_jobs.py --rescore` passes. Manual `Action` and `Priority` overrides are also preserved for manually-scored jobs.
...
- **Targeted Rescoring**: `--clear-score-override "<company_name>"` now passes target job IDs to `handle_rescore()` so only matching jobs are rescored instead of triggering a full active pipeline rescore.
- **Tolerant Search Queries**: Updated `query_jobs.py` SQLite query to normalize and strip apostrophes/curly quotes (e.g. `Lowes` automatically matches `Lowe's`).
- **Fixed Confidence/Reason Desync on Preserved Scores**: `clean_existing_tracker()` resolved a job's DB-corrected location (`stored_loc`) *after* calling `evaluate_job()`, so Rule 6 (Utah/Remote check) scored confidence and reason against a stale pre-sync location while `Fit Score`/`Recommendation` were separately pulled forward from the correct prior evaluation — producing rows like a 90/P1/Apply Now posting stamped with `🔴 Low` confidence and an "Out of state" reason. `stored_loc` is now resolved before `evaluate_job()` runs, so all four fields are computed from the same location.
- **Malformed-Company Validation**: `is_valid_company()` now rejects bare corporate-suffix fragments (e.g. `"ENTERPRISES, INC.."` with the identifying name missing), `"Your <X> Alert"` email-subject boilerplate, and job-title-shaped text ending in `"... at <Company>"` — all patterns from a PDF-layout wrap that let a truncated/garbage company name (`"1 message"` as location, alert-subject text as position) reach the tracker as a live P2 row.
- **Title Level Normalization (Roman Numerals)**: Added `normalize_title(title)` to `dedup_utils.py` mapping standalone Roman numeral level indicators (`I`, `II`, `III`, `IV`, `V`) to Arabic digits (`1`, `2`, `3`, `4`, `5`) in canonical deduplication keys (`canonical_key()`, `canonical_job_key()`). Roles like `Senior Developer I` and `Senior Developer 1` now map to identical deduplication keys.
- **`save_to_sqlite()` Write Filtering**: Added pre-write validity and deduplication gates to `save_to_sqlite()` to reject invalid auto-parsed company names (via `is_valid_company()`, while exempting CLI manual additions marked `_status_source == "user"`) and legacy-ID same-date duplicate rows before writing to `jobs.db`. Includes dedup snapshot reloading on schema drift retry.
