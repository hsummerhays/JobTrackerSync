...
- **Company Suffix Matching & Location Compatibility**: Updated `locations_compatible()` in `dedup_utils.py` to evaluate local Utah locations (e.g. `Midvale, UT` vs `West Valley City, UT`) for the same employer and position as compatible. Wrapped company suffix normalization (`normalize_company_for_matching()`) into ingestion duplicate matching and `clean_existing_tracker()`, resolving duplicate identity collisions (such as `Wheeler Machinery Co.` vs `Wheeler Machinery Company`).
